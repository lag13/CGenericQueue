...
*/
#define LQUEUE_CREATE_DATATYPE(name, type) struct name { struct { type data; void *next; } *head, *tail, *misc; size_t size; }

/*
Used to declare a variable of type queue by creating an
anonymous struct representing a queue.
*/
#define lqueue_t(type) struct { struct { type data; void *next; } *head, *tail, *misc; size_t size; }
...
*/
#define lqueue_remove(q) ((q).misc = (q).head, (q).head = (q).head->next, (q).size--, (q).misc->data)
